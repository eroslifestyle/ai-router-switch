#!/usr/bin/env python3
"""Monitor risposte vuote MiniMax - speculare a monitor_glm_empty.py.

Fonti:
  - router-usage.jsonl  righe mode=='minimax' con output_tokens<=5 (segnale primario)
  - debug-events.jsonl  eventi kind=='empty_response_minimax' (kind futuro, oggi assente)
...
# ponytail: raggruppamento case-insensitive. Nei log convivono 'MiniMax-M2.7',
# 'minimax-m2.7-hs', 'MiniMax-M3', 'minimax-m3', 'minimax'. Normalizziamo per
# non perdere righe solo per un cambio di casing.
def _norm_model(orig):
    if not orig or orig == '?':
        return '(altro)'
    o = orig.lower()
    if 'm3' in o and 'minimax' in o:
        return 'MiniMax-M3'
    if 'm2' in o and 'minimax' in o:
        return 'MiniMax-M2.7'
    if o == 'minimax':
        return 'MiniMax-M3'  # ponytail: 'minimax' senza versione = default M3 nel router
    return '(altro)'
...
def _rows_usage_since(ts_from):
    """Minimax rows with ts >= ts_from. Returns rows list + per-model aggregates."""
    rows = []
    by_model = {}
...
def _count_empty_events_since(ts_from_iso):
    """Conta kind=='empty_response_minimax' nella finestra. Oggi assente nei log."""
    n = 0
    with open(DEBUG_EVENTS, errors='replace') as f:
        for line in f:
            ...
            if e.get('ts', '') < ts_from_iso:
                continue
            if e.get('kind') == 'empty_response_minimax':
                n += 1
    return n
...
def report(max_rows=None):
    ...
    hdr = "{:>2} {:>8} {:>5} {:>8} {:>8} {:>9} {:>7}".format(
        '#', 'ora', 'min', 'tot min', 'empty', 'evt empt', '%vuote')
    print(hdr)
    print('-' * 60)
    gtot = gempty = gev = 0
    for i, s in enumerate(rows, 1):
        tot = s.get('total_minimax_requests', 0)
        emp = s.get('empty_output_le5', 0)
        gev_s = s.get('events_empty_response_minimax', 0)
        ...
        gempty += emp
        gev += gev_s
        # breakdown per modello (solo se ci sono modelli MiniMax nel campione)
        models = s.get('models', {})
        for mname in ('MiniMax-M3', 'MiniMax-M2.7'):
            md = models.get(mname)
            if not md or md.get('tot', 0) == 0:
                continue
            mpct = (100 * md['empty'] / md['tot']) if md['tot'] else 0.0
            print("      |__ {:<13} tot={:>5} empty={:>5} ({:.1f}%)".format(
    ...
    if gempty:
        print('AGGREGATO: {} empty(output<=5) / {} minimax ({:.1f}%) eventi={}'.format(
...
def sample():
    ...
    models_out = {m: by_model.get(m, {'tot': 0, 'empty': 0})
                  for m in ('MiniMax-M3', 'MiniMax-M2.7')}
    s = {
        'ts_start': ts_start, 'ts_end': now, 'ts_start_iso': ts_start_iso,
        'clock': _fmt_clock(now), 'window_min': window_min,
        'total_minimax_requests': total, 'empty_output_le5': empty,
        'events_empty_response_minimax': gev, 'models': models_out,
    ...
    if total == 0 and empty == 0:
        print('[{}] finestra {:.0f} min: NESSUNA richiesta minimax'.format(
            s['clock'], window_min))
        return
    print('[{}] {:.0f} min | minimax {} req | empty(ot<=5) {} ({:.1f}%) | evt {} | M2.7 {}/{} M3 {}/{}'.format(
        s['clock'], window_min, total, empty, pct, gev,
        models_out['MiniMax-M2.7']['empty'], models_out['MiniMax-M2.7']['tot'],
        models_out['MiniMax-M3']['empty'], models_out['MiniMax-M3']['tot']))
