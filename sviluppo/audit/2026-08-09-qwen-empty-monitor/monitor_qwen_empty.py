#!/usr/bin/env python3
"""Monitor risposte vuote Qwen - speculare a monitor_minimax_empty.py.

Fonti:
  - router-usage.jsonl  righe mode=='qwen' con output_tokens<=5 (segnale primario)
  - debug-events.jsonl  eventi kind=='empty_response_qwen' (kind futuro, oggi assente)
...
def _norm_model(orig):
    if not orig or orig == '?':
        return '(altro)'
    o = orig.lower()
    if '3.8' in o and 'max' in o:
        return 'qwen3.8-max'
    if 'coder' in o and 'plus' in o:
        return 'qwen3-coder-plus'
    if '3.7' in o and 'plus' in o:
        return 'qwen3.7-plus'
    return '(altro)'
    ...
    with open(DEBUG_EVENTS, errors='replace') as f:
        for line in f:
            ...
            if e.get('ts', '') < ts_from_iso:
                continue
            if e.get('kind') == 'empty_response_qwen':
                n += 1
    return n
...
def report(max_rows=None):
    ...
    hdr = "{:>2} {:>8} {:>5} {:>8} {:>8} {:>9} {:>7}".format(
        '#', 'ora', 'min', 'tot qwen', 'empty', 'evt empt', '%vuote')
    print(hdr)
    print('-' * 60)
    gtot = gempty = gev = 0
    for i, s in enumerate(rows, 1):
        tot = s.get('total_qwen_requests', 0)
        emp = s.get('empty_output_le5', 0)
        gev_s = s.get('events_empty_response_qwen', 0)
        ...
        gev += gev_s
        models = s.get('models', {})
        for mname in ('qwen3.8-max', 'qwen3-coder-plus', 'qwen3.7-plus'):
            md = models.get(mname)
            if not md or md.get('tot', 0) == 0:
                continue
            mpct = (100 * md['empty'] / md['tot']) if md['tot'] else 0.0
            print("      |__ {:<15} tot={:>5} empty={:>5} ({:.1f}%)".format(
    ...
    if gempty:
        print('AGGREGATO: {} empty(output<=5) / {} qwen ({:.1f}%) eventi={}'.format(
...
def sample():
    ...
    models_out = {m: by_model.get(m, {'tot': 0, 'empty': 0})
                  for m in ('qwen3.8-max', 'qwen3-coder-plus', 'qwen3.7-plus')}
    s = {
        'ts_start': ts_start, 'ts_end': now, 'ts_start_iso': ts_start_iso,
        'clock': _fmt_clock(now), 'window_min': window_min,
        'total_qwen_requests': total, 'empty_output_le5': empty,
        'events_empty_response_qwen': gev, 'models': models_out,
    ...
    if total == 0 and empty == 0:
        print('[{}] finestra {:.0f} min: NESSUNA richiesta qwen'.format(
            s['clock'], window_min))
        return
    print('[{}] {:.0f} min | qwen {} req | empty(ot<=5) {} ({:.1f}%) | evt {}'.format(
