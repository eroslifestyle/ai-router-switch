#!/usr/bin/env python3
"""Monitor risposte vuote Anthropic - speculare a monitor_minimax_empty.py.

Fonti:
  - router-usage.jsonl  righe mode=='anthropic' con output_tokens<=5 (segnale primario)
  - debug-events.jsonl  eventi kind=='empty_response_anthropic' (kind futuro, oggi assente)
    ...
    with open(DEBUG_EVENTS, errors='replace') as f:
        for line in f:
            ...
            if e.get('ts', '') < ts_from_iso:
                continue
            if e.get('kind') == 'empty_response_anthropic':
                n += 1
    return n
...
def report(max_rows=None):
    ...
    if max_rows:
        rows = rows[-max_rows:]
    hdr = "{:>2} {:>8} {:>5} {:>10} {:>8} {:>9} {:>7}".format(
        '#', 'ora', 'min', 'tot anthropic', 'empty', 'evt empt', '%vuote')
    print(hdr)
    print('-' * 65)
    gtot = gempty = gev = 0
    for i, s in enumerate(rows, 1):
        tot = s.get('total_anthropic_requests', 0)
        emp = s.get('empty_output_le5', 0)
        gev_s = s.get('events_empty_response_anthropic', 0)
        pct = s.get('pct_empty_of_total', 0.0)
        print("{:>2} {:>8} {:>5.0f} {:>10} {:>8} {:>9} {:>6.1f}%".format(
            i, s.get('clock', ''), s.get('window_min', 0), tot, emp, gev_s, pct))
        gtot += tot
        gempty += emp
        gev += gev_s
        models = s.get('models', {})
        for mname in ('claude-opus-5', 'claude-sonnet-5', 'claude-haiku-4-5', 'claude-fable-5'):
            md = models.get(mname)
            if not md or md.get('tot', 0) == 0:
                continue
            mpct = (100 * md['empty'] / md['tot']) if md['tot'] else 0.0
            print("      |__ {:<18} tot={:>6} empty={:>5} ({:.1f}%)".format(
                mname, md['tot'], md['empty'], mpct))
    print('-' * 65)
    agg_pct = (100 * gempty / gtot) if gtot else 0.0
    print("{:>3} {:>8} {:>5} {:>10} {:>8} {:>9} {:>6.1f}%".format(
        'TOT', '', '', gtot, gempty, gev, agg_pct))
    if gempty:
        print('AGGREGATO: {} empty(output<=5) / {} anthropic ({:.1f}%) eventi={}'.format(
...
def sample():
    ...
    models_out = {m: by_model.get(m, {'tot': 0, 'empty': 0})
                  for m in ('claude-opus-5', 'claude-sonnet-5', 'claude-haiku-4-5', 'claude-fable-5')}
    s = {
        'ts_start': ts_start, 'ts_end': now, 'ts_start_iso': ts_start_iso,
        'clock': _fmt_clock(now), 'window_min': window_min,
        'total_anthropic_requests': total, 'empty_output_le5': empty,
        'events_empty_response_anthropic': gev, 'models': models_out,
    ...
    if total == 0 and empty == 0:
        print('[{}] finestra {:.0f} min: NESSUNA richiesta anthropic'.format(
            s['clock'], window_min))
        return
    print('[{}] {:.0f} min | anthropic {} req | empty(ot<=5) {} ({:.1f}%) | evt {}'.format(
